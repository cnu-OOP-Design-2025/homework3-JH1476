...
int numOfStudentReference = 0;
int& numOfStudent = numOfStudentReference;
...
struct SortByName{
    char operator()(const StudentStruct& a, const StudentStruct& b) const{
...
struct SortByMidterm{
    float operator()(const StudentStruct& a, const StudentStruct& b) const{
...
void doTest2(){
    ...
    cout << "Sort by Name" << endl;
    auto sortByName = [](StudentStruct &a, StudentStruct &b) -> char {
        return a.name < b.name;
    };
    std::sort(students, students + numOfStudent, sortByName);
    printStudentList();
    ...
    cout << "Sort by Midterm" << endl;
    auto sortByMidterm = [](StudentStruct &a, StudentStruct &b) -> float {
        return a.record.midterm > b.record.midterm;
    };
    std::sort(students, students + numOfStudent, sortByMidterm);
    printStudentList();
    ...
    cout << "Sort by Final" << endl;
    auto sortByFinal = [](StudentStruct &a, StudentStruct &b) -> float {
        return a.record.final > b.record.final;
    };
    std::sort(students, students + numOfStudent, sortByFinal);
    printStudentList();
    ...
    cout << " Sort by Total" << endl;
    auto sortByTotal = [](StudentStruct &a, StudentStruct &b) -> float {
        return a.record.midterm + a.record.final > b.record.midterm + b.record.final;
    };
    std::sort(students, students + numOfStudent, sortByTotal);
    printStudentList();
...
int main(int argc, char **argv) {
    ...
    /* student 파라미터는 포인터 변수이기 때문에 Reference로 전달할 수 없습니다! */
